...
def render_backtest() -> None:
    ...
    with col1:
        ...
        period_type = st.selectbox("기간 설정:", ["최근 N일", "날짜 직접 지정"])
        ...
        strategy = st.selectbox(
            "매매 전략:", ["MACD 전략", "RSI 전략", "볼린저 밴드 전략", "이동평균 전략"]
        )
            ...
            with st.spinner("백테스팅 실행 중..."):
                ...
                if results:
                    ...
                    with col2:
                        ...
                        st.metric(
                            "승률", f"{win_rate:.1f}%", delta=f"{win_rate-50:.1f}%"
                        )
                    with col3:
                    ...
                    if "trades" in results:
                        ...
                        st.write(f"거래 내역 shape: {trades_df.shape}")  # 디버그용
                        if not trades_df.empty:
                            st.dataframe(trades_df.tail(20), use_container_width=True)
                        else:
                            st.warning("실제 거래가 발생하지 않았습니다. 신호는 생성되었으나 모든 거래가 조건에 의해 무시되었습니다.")
                else:
                    st.error("❌ 백테스팅 실행 실패")
