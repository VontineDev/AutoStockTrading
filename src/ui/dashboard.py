...
def render_dashboard() -> None:
    ...
    # 헤더
    st.markdown(
        f'<h1 class="main-header">📈 {config["project"]["name"]}</h1>',
        unsafe_allow_html=True,
    )
    st.markdown(
        f'<p style="text-align: center; color: #666;">v{config["project"]["version"]} | 100만원 규모 스윙 트레이딩 시스템</p>',
        unsafe_allow_html=True,
    )
    ...
    period_options = {"1개월": 30, "3개월": 90, "6개월": 180, "1년": 365, "2년": 730}

    selected_period = st.sidebar.selectbox(
        "기간 선택", list(period_options.keys()), index=2
    )
    ...
    if missing_cols:
        st.error(
            f"데이터에 {missing_cols} 컬럼이 없습니다. 실제 컬럼: {df.columns.tolist()}"
        )
        return
    ...
    with col2:
        ...
        volume_change_pct = (
            (volume_change / prev["volume"]) * 100 if prev["volume"] > 0 else 0
        )
    ...
    with col3:
        if "RSI" in df.columns and not pd.isna(latest["RSI"]):
            ...
            st.metric(label="RSI (14)", value=f"{rsi_value:.1f}", delta=rsi_status)
        else:
    ...
    with col4:
        if "MACD" in df.columns and not pd.isna(latest["MACD"]):
            ...
            st.metric(
                label="MACD 신호", value=f"{latest['MACD']:.3f}", delta=macd_signal
            )
        else:
    ...
    chart = create_candlestick_chart(
        df, f"{selected_symbol} ({symbol_names.get(selected_symbol, '')})"
    )
