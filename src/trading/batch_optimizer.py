...
def adaptive_batch_sizing(
    self, processing_time: float, batch_size: int, success_rate: float
):
    """적응형 배치 크기 조정"""
    ...
    if len(self.performance_history) >= 2:
        ...
        if memory_ratio > 0.8:  # 메모리 부족 시 배치 크기 감소
            ...
            logger.info(
                f"메모리 부족으로 배치 크기 감소: {batch_size} -> {new_batch_size}"
            )
        elif (
            recent_performance > previous_performance * 1.1
        ):  # 성능 향상 시 배치 크기 증가
            new_batch_size = min(self.config.max_batch_size, int(batch_size * 1.2))
            logger.info(
                f"성능 향상으로 배치 크기 증가: {batch_size} -> {new_batch_size}"
            )
        elif (
            recent_performance < previous_performance * 0.9
        ):  # 성능 저하 시 배치 크기 감소
...
def optimize_memory_usage(self):
    ...
    if self.memory_monitor.is_memory_exceeded():
        ...
        if new_batch_size != self.current_batch_size:
            logger.warning(
                f"메모리 한계 초과로 배치 크기 감소: {self.current_batch_size} -> {new_batch_size}"
            )
            self.current_batch_size = new_batch_size
            self.stats["batch_size_changes"] += 1
...
def process_batch_with_optimization(
    self, batch: List[Any], processor_func: callable, **kwargs
) -> Dict[str, Any]:
    """최적화된 배치 처리"""
    ...
    if self.stats["total_time"] > 0:
        self.stats["avg_items_per_second"] = (
...
def load_data_batch(
    self, symbols: List[str], data_source: callable
...
def process_symbols_in_batches(
    ...
    **kwargs,
    ...
    for batch_idx, batch in enumerate(batches):
        logger.info(
            f"배치 {batch_idx + 1}/{len(batches)} 처리 중... ({len(batch)}개 심볼)"
        )
        ...
        logger.info(
            f"진행률: {progress:.1f}% ({batch_idx + 1}/{len(batches)} 배치 완료)"
        )
    ...
    logger.info(
        f"배치 처리 완료: {successful_count}/{len(symbols)} 성공, {total_time:.2f}초"
    )

    return {"results": all_results, "performance_stats": performance_stats}
...
def optimize_for_system(self) -> Dict[str, Any]:
    ...
    recommended_memory_limit = min(
        available_memory_mb * 0.6, 2048
    )  # 60% 또는 2GB 중 작은 값
    recommended_batch_size = max(
        10, int(recommended_memory_limit / 20)
    )  # 20MB당 1개 배치
...
def create_optimized_batch_processor(
    max_memory_mb: int = None, batch_size: int = None
) -> BatchProcessor:
    """최적화된 배치 처리기 생성"""
