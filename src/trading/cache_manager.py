...
class BacktestCacheManager:
    ...
    def get_data_hash(self, data: pd.DataFrame) -> str:
        ...
        if len(data) > 10:
            ...
            data_info['mid_hash'] = hashlib.md5(mid_data.to_string().encode()).hexdigest()
    ...
    def get_cached_result(self,
                         ...
                         data: pd.DataFrame,
                         strategy_params: Dict = None,
                         backtest_config: Dict = None) -> Optional[Dict[str, Any]]:
        """캐시된 결과 조회"""
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                if result:
                    ...
                    if datetime.now() - created_time < max_age:
                        ...
                        # 만료된 캐시 삭제
                        cursor.execute('DELETE FROM backtest_cache WHERE cache_key = ?', (cache_key,))
                        conn.commit()
    ...
    def _add_to_memory_cache(self, cache_key: str, results: Dict[str, Any]):
        ...
        with self.cache_lock:
            ...
            while len(self.memory_cache) >= self.config.memory_cache_size:
                # 가장 오래된 항목 제거 (LRU)
                oldest_key = min(self.memory_cache_usage.keys(), 
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if stats['total_requests'] > 0:
                ...
                stats['miss_rate'] = (stats['cache_misses'] / stats['total_requests']) * 100
