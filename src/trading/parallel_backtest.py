...
class ParallelBacktestEngine:
    ...
    def run_parallel_backtest(self, 
        ...
        logger.info(f"병렬 백테스팅 시작: {len(symbols)}개 종목, {self.config.max_workers}개 워커")
            ...
            with ProcessPoolExecutor(max_workers=self.config.max_workers) as executor:
                ...
                for future in as_completed(future_to_chunk, timeout=self.config.timeout):
                    ...
                    try:
                        ...
                        # 성공한 백테스팅 수 업데이트
                        successful_count = sum(1 for r in chunk_results.values() if r.get('success', False))
                        self.performance_stats['successful_backtests'] += successful_count
                        self.performance_stats['failed_backtests'] += len(chunk_results) - successful_count
                        ...
                        if self.config.progress_callback:
                            ...
                            self.config.progress_callback(progress, completed_chunks, len(tasks))
                        
                        logger.info(f"청크 {chunk_id} 완료: {len(chunk_results)}개 종목 처리")
                        
                    except Exception as e:
                        logger.error(f"청크 {chunk_id} 처리 실패: {e}")
                        self.performance_stats['failed_backtests'] += len(tasks[chunk_id]['symbols_data'])
    ...
    def get_performance_report(self) -> Dict[str, Any]:
        ...
        if stats['total_symbols'] > 0:
            stats['success_rate'] = stats['successful_backtests'] / stats['total_symbols'] * 100
...
def progress_callback_default(progress: float, completed: int, total: int):
    """기본 진행률 콜백"""
    print(f"\r진행률: {progress:.1f}% ({completed}/{total} 청크 완료)", end='', flush=True)
    if completed == total:
        print()  # 완료 시 새 줄

# 편의 함수들
def run_parallel_backtest_simple(strategy_class, symbols_data: Dict[str, pd.DataFrame], 
...
def benchmark_parallel_performance(strategy_class, sample_data: Dict[str, pd.DataFrame]) -> Dict[str, Any]:
    """병렬 처리 성능 벤치마크"""
    results = {}
