...
class Portfolio:
    ...
    def buy(self, symbol: str, quantity: int, price: float, date: datetime = None) -> bool:
        ...
        self.logger.info(
            f"매수 완료: {symbol} {quantity}주 @ {price:,.0f}원 (수수료: {commission:,.0f}원)"
        )
        return True
    ...
    def sell(self, symbol: str, quantity: int, price: float, date: datetime = None) -> bool:
        ...
        if position.quantity < quantity:
            self.logger.warning(
                f"보유 수량 부족: 요청 {quantity}주, 보유 {position.quantity}주"
            )
            return False
        ...
        self.logger.info(
            f"매도 완료: {symbol} {quantity}주 @ {price:,.0f}원 (수수료: {commission:,.0f}원, 세금: {tax:,.0f}원)"
        )
        return True
            ...
            data.append(
                {
                    ...
                    "세금": trade.tax,
                    "순거래금액": (
                        trade.net_amount if trade.action == "SELL" else -trade.net_amount
                    ),
                }
            )
    ...
    def get_performance_metrics(self) -> Dict[str, float]:
        ...
        for trade in self.trades:
            if trade.action == "SELL":
                ...
                for prev_trade in reversed(self.trades):
                    if (
                        prev_trade.symbol == trade.symbol
                        and prev_trade.action == "BUY"
                        and prev_trade.date < trade.date
                    ):
        ...
        win_rate = (
            profitable_trades / total_completed_trades if total_completed_trades > 0 else 0
        )
