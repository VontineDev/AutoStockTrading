...
class Trade:
    ...
    def profit_loss(self) -> float:
        ...
        return (
            ...
            * (1 if self.trade_type == "LONG" else -1)
        )
...
class BacktestConfig:
    ...
    position_size_method: str = (
        "fixed_amount"  # 'fixed_amount', 'percent', 'volatility'
    )
...
class BacktestEngine:
    ...
    def __init__(self, config: BacktestConfig = None):
        ...
        self.reset()
        logger.info(
            f"백테스팅 엔진 초기화: 초기자본 {self.config.initial_capital:,.0f}원"
        )
    ...
    def run_backtest(
        ...
        Returns:
            백테스팅 결과
        """
        logger.info(f"run_backtest 진입: strategy={strategy}")
        ...
        for symbol, df in data.items():
            ...
            if "date" in df.columns:
                ...
                all_dates.update(
                    pd.to_datetime(df.index, format="mixed", errors="coerce").date
                )
        ...
        if start_date:
            start_dt = pd.to_datetime(
                start_date, format="mixed", errors="coerce"
            ).date()
        ...
        logger.info(
            f"백테스팅 완료: 총 {len(self.trades)}개 거래, 최종 수익률 {results['total_return']:.2%}"
        )
        ...
        for symbol, df in data.items():
            ...
            if "date" in df.columns:
                day_data = (
                    ...
                    else df[df["date"] == date]
                )
            else:
                # date 컬럼이 없으면 인덱스 기반
                day_data = (
                    df[df.index.date == date]
    ...
    def _process_buy_signal(self, signal, current_price: float, market_data: pd.Series):
        ...
        if symbol in self.positions:
            logger.info(f"{symbol}: 이미 포지션 보유 중, 매수 신호 무시 (신호: {signal})")
            return

        # 최대 포지션 수 확인
        if len(self.positions) >= self.config.max_positions:
            logger.info(f"{symbol}: 최대 포지션 수 초과({self.config.max_positions}), 매수 신호 무시 (신호: {signal})")
            return

        # 포지션 크기 계산
        position_size = self._calculate_position_size(
            current_price, signal, market_data
        )

        if position_size <= 0:
            logger.info(f"{symbol}: 포지션 크기 0 이하({position_size}), 매수 신호 무시 (신호: {signal})")
            return
        ...
        if total_cost > self.cash:
            logger.info(f"{symbol}: 자금 부족 (필요: {total_cost:.0f}, 보유: {self.cash:.0f}), 매수 신호 무시 (신호: {signal})")
            # 자금 부족 시 가능한 최대 수량으로 조정
            available_value = self.cash * 0.95  # 5% 버퍼
            adjusted_trade_value = available_value / (
            ...
            if position_size < 1:  # 최소 거래 단위
                logger.info(f"{symbol}: 자금 부족으로 최소 거래 단위 미만, 매수 신호 최종 무시 (신호: {signal})")
                return

        # 손절/익절 가격 계산
        stop_loss = current_price * (1 - self.config.risk_per_trade)
        take_profit = current_price * (
            1 + self.config.risk_per_trade * 2
        )  # 리워드:리스크 = 2:1
        ...
        logger.info(
            f"매수 실행: {symbol} {position_size:.2f}주 @ {current_price:,.0f}원 (신호: {signal})"
        )

    def _process_sell_signal(
        self, signal, current_price: float, market_data: pd.Series
    ):
        """매도 신호 처리"""
        symbol = signal.symbol
        ...
        if symbol not in self.positions:
            logger.info(f"{symbol}: 보유 포지션 없음, 매도 신호 무시 (신호: {signal})")
            return

        position = self.positions[symbol]
        self._close_position(symbol, current_price, signal.reason)
        logger.info(f"매도 실행: {symbol} {position.quantity:.2f}주 @ {current_price:,.0f}원 (신호: {signal})")

    def _close_position(self, symbol: str, exit_price: float, exit_reason: str):
        """포지션 청산"""
        if symbol not in self.positions:
            logger.debug(f"{symbol}: 청산 시도했으나 포지션 없음 (exit_price: {exit_price}, reason: {exit_reason})")
            return
        ...
        logger.debug(
            f"매도 실행: {symbol} {position.quantity:.2f}주 @ {exit_price:,.0f}원, 수익률: {trade.return_pct:.2%}"
        )

    def _calculate_position_size(
        self, price: float, signal, market_data: pd.Series
    ) -> float:
        """포지션 크기 계산"""
        # 상세 로그 추가
        logger.debug(f"포지션 크기 계산: price={price}, signal={signal}, market_data={market_data.to_dict() if hasattr(market_data, 'to_dict') else market_data}")
        if self.config.position_size_method == "fixed_amount":
            ...
            if "ATR" in market_data:
                ...
                logger.debug(f"volatility 방식: ATR={atr}, risk_amount={risk_amount}, position_size={position_size}")
    ...
    def _close_all_positions(self, data: Dict[str, pd.DataFrame], final_date):
        ...
        for symbol in list(self.positions.keys()):
            if symbol in data:
                ...
                if "date" in df.columns:
                    final_data = (
                        df[df["date"].dt.date == final_date]
                        if hasattr(df["date"], "dt")
                        else df[df["date"] == final_date]
                    )
                else:
                    final_data = (
                        df[df.index.date == final_date]
                        if hasattr(df.index, "date")
                        else df.iloc[[-1]]
                    )
    ...
    def _analyze_results(self) -> Dict[str, Any]:
        ...
        # 수익률 통계
        total_return = (
            self.get_portfolio_value() - self.config.initial_capital
        ...
        results = {
            ...
            "max_holding_days": (
                max([trade.holding_days for trade in self.trades]) if self.trades else 0
            ),
