...
def _generate_buy_signals(self, df: pd.DataFrame) -> List[TradeSignal]:
    ...
    for i in range(len(df)):
        ...
        # 조건 3: 강세 배열에서 가격이 빠른 이동평균 위로 복귀
            ...
            current_row.get("bullish_alignment", False)
            and current_row.get("price_above_fast", False)
            and not df.iloc[i - 1]["price_above_fast"]
            if i > 0
            else False
        ):
            buy_conditions.append(True)
...
def _generate_sell_signals(self, df: pd.DataFrame) -> List[TradeSignal]:
    ...
    for i in range(len(df)):
        ...
        # 조건 3: 약세 배열에서 가격이 빠른 이동평균 아래로 이탈
            ...
            current_row.get("bearish_alignment", False)
            and not current_row.get("price_above_fast", False)
            and df.iloc[i - 1]["price_above_fast"]
            if i > 0
            else False
        ):
            sell_conditions.append(True)
...
def generate_signals(self, data: pd.DataFrame) -> list:
    """Moving Average 신호 생성: 데이터 컬럼 체크 및 예외 발생 시 빈 리스트 반환, 상세 로깅"""
    logger = logging.getLogger(__name__)
    logger.debug(f"[MovingAverageStrategy] 입력 데이터 shape: {data.shape}, 컬럼: {list(data.columns)}")
    required_cols = ['close']
    for col in required_cols:
        if col not in data.columns:
            logger.error(f"[MovingAverageStrategy] 필수 컬럼 누락: {col}")
            return []
    try:
        df = data.copy()
        ma = talib.SMA(df['close'], timeperiod=20)
        df['MA'] = ma
        logger.debug(f"[MovingAverageStrategy] MA 컬럼 생성 여부: {'MA' in df.columns}, 첫 5개: {df['MA'].head().tolist()}")
        signals = []
        # 지표 계산
        ...
        logger.info(f"MA 전략 신호 생성: {len(all_signals)}개 (데이터 shape: {df.shape})")
        logger.debug(f"[MovingAverageStrategy] 생성된 신호 수: {len(all_signals)}")
        return all_signals
    except Exception as e:
        logger.error(f"[MovingAverageStrategy] 신호 생성 중 예외 발생: {e}")
        return []
...
def validate_signal(self, signal: TradeSignal, data: pd.DataFrame) -> bool:
    ...
    atr_ratio = (
        signal.indicators.get("ATR", 0) / signal.price if signal.price > 0 else 0
    )
...
def create_moving_average_strategy(
    short_period: int = 5, long_period: int = 20, use_filters: bool = True
) -> "MovingAverageStrategy":
    # Implementation of create_moving_average_strategy method
    pass
