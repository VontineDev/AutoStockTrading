...
from .base_strategy import BaseStrategy, TradeSignal, StrategyConfig, create_default_config, calculate_signal_confidence
...
def calculate_indicators(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.volume_filter:
            df['volume_ma'] = talib.SMA(df['volume'], timeperiod=self.parameters.get('volume_ma_period', 20))
...
def generate_signals(self, data: pd.DataFrame) -> List[TradeSignal]:
        ...
        for i in range(len(data)):
            ...
            # 2. MACD 히스토그램 상승 전환
            if (current_row['MACD_hist'] > self.histogram_threshold and 
                prev_row['MACD_hist'] <= self.histogram_threshold and
                current_row['hist_change'] > 0):
                buy_conditions.append("히스토그램 상승 전환")
            
            # 3. MACD 라인이 0선 위에서 상승
            if (current_row['MACD'] > 0 and current_row['MACD_change'] > 0 and
...
def _calculate_buy_confidence(self, row: pd.Series) -> float:
        ...
        base_confidence = calculate_signal_confidence(indicators, self.confidence_thresholds)
...
def create_macd_strategy(fast: int = 12, slow: int = 26, signal: int = 9,
                        use_filters: bool = True) -> MACDStrategy:
    """MACD 전략 팩토리 함수"""
