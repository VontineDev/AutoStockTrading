...
class TALibIndicators:
    ...
    def calculate_trend_indicators(self) -> pd.DataFrame:
        ...
        # 이동평균선 (SMA)
        df['SMA_5'] = talib.SMA(df['close'], timeperiod=SWING_TRADING_PARAMS['SMA']['short'])
        df['SMA_20'] = talib.SMA(df['close'], timeperiod=SWING_TRADING_PARAMS['SMA']['medium'])
        df['SMA_60'] = talib.SMA(df['close'], timeperiod=SWING_TRADING_PARAMS['SMA']['long'])
        
        # 지수이동평균선 (EMA)
        df['EMA_12'] = talib.EMA(df['close'], timeperiod=SWING_TRADING_PARAMS['EMA']['short'])
        df['EMA_26'] = talib.EMA(df['close'], timeperiod=SWING_TRADING_PARAMS['EMA']['medium'])
        df['EMA_50'] = talib.EMA(df['close'], timeperiod=SWING_TRADING_PARAMS['EMA']['long'])
    ...
    def calculate_volatility_indicators(self) -> pd.DataFrame:
        ...
        df['ATR'] = talib.ATR(df['high'], df['low'], df['close'], timeperiod=atr_params['period'])
    ...
    def calculate_all_indicators(self) -> pd.DataFrame:
        ...
        try:
            ...
            logger.info(f"총 {len(indicator_columns)}개 지표 계산 완료: {indicator_columns}")
            return df
    ...
    def get_trading_signals(self, df: pd.DataFrame = None) -> pd.DataFrame:
        ...
        # MACD 신호 (골든크로스/데드크로스)
        signals_df['MACD_buy'] = (df['MACD'] > df['MACD_signal']) & (df['MACD'].shift(1) <= df['MACD_signal'].shift(1))
        signals_df['MACD_sell'] = (df['MACD'] < df['MACD_signal']) & (df['MACD'].shift(1) >= df['MACD_signal'].shift(1))
        ...
        # 이동평균 골든크로스/데드크로스
        signals_df['MA_golden_cross'] = (df['SMA_5'] > df['SMA_20']) & (df['SMA_5'].shift(1) <= df['SMA_20'].shift(1))
        signals_df['MA_dead_cross'] = (df['SMA_5'] < df['SMA_20']) & (df['SMA_5'].shift(1) >= df['SMA_20'].shift(1))
...
def calculate_custom_indicators(data: pd.DataFrame, params: Dict = None) -> pd.DataFrame:
    """
    사용자 정의 매개변수로 지표 계산
