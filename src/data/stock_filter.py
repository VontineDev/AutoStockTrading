    ...
    for attempt_date in dates_to_try:
        try:
            ...
            if market == "KOSPI":
                ...
                if top_stocks:
                    ...
                    if min_cap:
                        ...
                        logger.warning(
                            "StockDataUpdater는 현재 min_cap 필터링을 직접 지원하지 않습니다."
                        )
                    ...
                    # 데이터베이스 캐시 저장
                    trading_calendar.cache_market_data(
                        attempt_date, market, "market_cap", top_stocks
                    )

                    logger.info(
                        f"시가총액 상위 {len(top_stocks)}개 종목 조회 완료 ({attempt_date})"
                    )
                    return top_stocks
                else:
                    logger.warning(
                        f"StockDataUpdater에서 KOSPI 시가총액 데이터 없음: {attempt_date}"
                    )
                    continue
            ...
            # KOSDAQ 또는 ALL은 기존 pykrx 로직 유지
                ...
                kosdaq_df = stock.get_market_cap_by_ticker(
                    attempt_date, market="KOSDAQ"
                )
...
def get_volume_top(
    ...
    for attempt_date in dates_to_try:
        try:
            ...
            if market == "KOSPI":
                all_symbols_in_market = self.data_updater.get_kospi_symbols(
                    date=attempt_date
                )
            elif market == "KOSDAQ":
                all_symbols_in_market = self.data_updater.get_kosdaq_symbols(
                    date=attempt_date
                )
            elif market == "ALL":
                all_symbols_in_market = self.data_updater.get_kospi_symbols(
            ...
            if not all_symbols_in_market:
                logger.warning(
                    f"해당 시장에 종목 데이터 없음: {market} ({attempt_date})"
                )
                continue
            ...
            # 데이터베이스 캐시 저장
            trading_calendar.cache_market_data(
                attempt_date, market, "volume", top_stocks
            )

            logger.info(
                f"거래량 상위 {len(top_stocks)}개 종목 조회 완료 ({attempt_date})"
            )
            return top_stocks
...
def get_price_change_top(
    ...
    # 캐시된 데이터 확인
    cached_data = trading_calendar.get_cached_market_data(
        date, market, f"price_change_{change_type}"
    )
    if cached_data:
        logger.info(f"데이터베이스 캐시 사용: {market} ({date})")
        return cached_data[:top_n]
    ...
    for attempt_date in dates_to_try:
        try:
            ...
            change_data = []
            for symbol in all_symbols_in_market:
                df = self.data_updater.fetch_stock_data(
                    symbol, attempt_date, attempt_date
                )
                if df is not None and not df.empty:
                    # 등락률 계산 (당일 종가 기준)
                    ...
                    if df["open"].iloc[0] > 0:
                        change_rate = (
                            (df["close"].iloc[0] - df["open"].iloc[0])
                            / df["open"].iloc[0]
                            * 100
                        )
                    else:
            ...
            # 데이터베이스 캐시 저장
            trading_calendar.cache_market_data(
                attempt_date, market, f"price_change_{change_type}", top_stocks
            )

            logger.info(
                f"등락률 상위 {len(top_stocks)}개 종목 조회 완료 ({attempt_date})"
            )
            return top_stocks
        ...
        try:
            ...
            for symbol in symbols:
                try:
                    ...
                    if not symbol_info:
                        logger.warning(
                            f"종목 정보 조회 실패 (StockDataUpdater): {symbol}"
                        )
                        continue
                    name = symbol_info["name"]

                    # 기본 정보 조회 (StockDataUpdater 사용)
                    ohlcv_df = self.data_updater.fetch_stock_data(
                        symbol, attempt_date, attempt_date
                    )
...
def get_kosdaq_top(n: int = 30, by: str = "market_cap", **kwargs) -> List[str]:
    ...
    if by == "market_cap":
        ...
        logger.warning(
            "StockDataUpdater에 KOSDAQ 시가총액 상위 기능이 없어 전체 종목에서 필터링합니다."
        )
...
if __name__ == "__main__":
    ...
    if market_cap_stocks:
        ...
        if not info_df.empty:
            print(
                info_df[
                    ["symbol", "name", "close", "market_cap_billion", "change_rate"]
