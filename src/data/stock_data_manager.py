...
def calculate_and_save_indicators(self, stock_code):
        ...
        selected_columns.extend(['ma_5', 'ma_10', 'ma_20', 'ma_60', 'ma_slope_5', 'ma_slope_10', 'ma_slope_20'])

        # 실제 df에 존재하는 컬럼만 선택
        # df에 없는 컬럼은 selected_columns에서 제거
        final_selected_columns = [col for col in selected_columns if col in df.columns]
        df_indicators = df[final_selected_columns].copy() # .copy()를 사용하여 원본 DataFrame에 영향 주지 않음
        
        self.db.execute("DELETE FROM technical_indicators WHERE symbol = ?", (stock_code,))
        with sqlite3.connect(self.db.db_path) as conn:
            df_indicators.dropna().to_sql('technical_indicators', conn, if_exists='append', index=False)
        return True
