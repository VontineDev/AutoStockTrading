...
def connect(self) -> None:
        ...
        self.ws = await websockets.connect(url, extra_headers={"authorization": f"Bearer {token}"})
...
def receive_messages(self) -> None:
    ...
    async @log_function_trace
def subscribe(self, tr_types: List[str], items: Optional[List[str]] = None, grp_no: str = "1", refresh: str = "1") -> None:
    ...
    async @log_function_trace
def unsubscribe(self, tr_types: List[str], items: Optional[List[str]] = None, grp_no: str = "1") -> None:
    ...
    async @log_function_trace
def run(self, tr_types: List[str], items: Optional[List[str]] = None) -> None:
        await self.connect()
