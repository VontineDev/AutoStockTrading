...
def load_config() -> dict:
    ...
    # 기본 설정 반환
    return {
        "project": {"name": "TA-Lib 스윙 트레이딩", "version": "1.0.0"},
        "logging": {"level": "INFO"},
    }
...
def setup_environment():
    ...
    try:
        ...
        logger.warning(
            "python-dotenv가 설치되지 않았습니다. 환경변수 로드를 건너뜁니다."
        )
...
def check_dependencies():
    ...
    required_packages = [
        ...
        ("pandas", "pandas"),
        ("numpy", "numpy"),
        # ('pykrx', 'pykrx'), # StockDataUpdater에서 관리
        ("streamlit", "streamlit"),
        ("plotly", "plotly"),
        ("sqlite3", "sqlite3 (내장)"),
    ]
...
def parse_date(date_str: str) -> str:
    ...
    try:
        ...
        raise ValueError(
            f"잘못된 날짜 형식: {date_str}. YYYY-MM-DD 또는 YYYYMMDD 형식을 사용하세요."
        )
...
def run_data_update(args):
    ...
    try:
        ...
        from src.data.stock_data_manager import (
            StockDataManager,
        )  # StockDataManager 임포트 추가

        updater = StockDataUpdater()
        dm = StockDataManager(db_path=updater.db_path)  # StockDataManager 인스턴스 생성
        ...
        if hasattr(args, "summary") and args.summary:
            ...
            display_data_summary(updater, include_backtest)
            return
        ...
        if hasattr(args, "yesterday_only") and args.yesterday_only:
            ...
            if hasattr(args, "all_kospi") and args.all_kospi:
                ...
                logger.info(
                    f"코스피 전체 {len(all_symbols)}개 종목 전날 데이터 업데이트"
                )
            ...
            logger.info(
                f"- 성공: {results['success_count']}/{results['total_symbols']} 종목"
            )
        ...
        try:
            ...
            if hasattr(args, "start_date") and args.start_date:
                logger.info(
                    f"날짜 설정: 직접 지정 ({args.start_date} ~ {getattr(args, 'end_date', '오늘')})"
                )
            elif hasattr(args, "days") and args.days:
                ...
                logger.info(
                    f"날짜 설정: 기본 기간 ({period_names.get(period, period)})"
                )
        ...
        if hasattr(args, "yesterday_only") and not args.yesterday_only:
            if len(symbols) > 10 and days_diff > 30:
                ...
                logger.info(
                    "전날 데이터만 필요한 경우 --yesterday-only 옵션을 사용하세요 (Ultra-Fast, 4-5초 완료)"
                )
        ...
        if hasattr(updater, "update_multiple_symbols"):
            ...
            # --- 추가: 기술적 지표 계산 및 저장 ---
            logger.info("\n=== 기술적 지표 계산 및 저장 시작 ===")
            indicators_success_count = 0
            for symbol in symbols:
                if results.get(symbol, False):  # 데이터 수집 성공한 종목만 지표 계산
                    try:
                        if dm.calculate_and_save_indicators(symbol):
                            indicators_success_count += 1
                            logger.debug(f"✅ {symbol}: 지표 계산 및 저장 완료")
                        else:
                            logger.warning(
                                f"⚠️ {symbol}: 지표 계산 실패 또는 데이터 부족"
                            )
                    except Exception as e:
                        logger.error(f"❌ {symbol}: 지표 계산 중 오류 발생 - {e}")
            logger.info(
                f"✅ 기술적 지표 계산 및 저장 완료: {indicators_success_count}/{len(symbols)} 종목"
            )
            # --- 추가 끝 ---
            ...
            for i, symbol in enumerate(symbols, 1):
                ...
                try:
                    ...
                    try:
                        if dm.calculate_and_save_indicators(symbol):
                            logger.debug(f"  ✅ 지표 계산 및 저장 완료")
                        else:
                            logger.warning(f"  ⚠️ 지표 계산 실패 또는 데이터 부족")
                    except Exception as e:
...
def run_backtest(args):
    ...
    try:
        ...
        if args.all_kospi:
            ...
            # 코스피 상위 종목 가져오기 (StockDataUpdater 사용)
            try:
                from scripts.data_update import StockDataUpdater
                ...
                logger.warning(
                    f"StockDataUpdater를 통한 코스피 상위 종목 조회 실패: {e}. 데이터베이스에서 가져옵니다."
                )
        ...
        logger.info(
            f"📅 백테스팅 기간: {start_date.strftime('%Y-%m-%d')} ~ {end_date.strftime('%Y-%m-%d')} ({args.days}일)"
        )
            ...
            for symbol in symbols:
                ...
                try:
                    ...
                    if not df.empty and len(df) >= 20:  # 최소 20일 데이터로 완화
                        ...
                        logger.info(
                            f"❌ {symbol}: 데이터 부족 ({len(df) if not df.empty else 0}건)"
                        )
...
def run_check_data(args):
    ...
    try:
        ...
        # 분석 매개변수 설정 (display_data_summary에서 사용하지 않으므로 제거)
        ...
        # 추가 권장사항
        # 이 부분은 display_data_summary 함수에서 반환하는 값을 활용하거나,
        # StockDataUpdater에서 직접 가져와야 합니다.
        # 현재는 StockDataUpdater의 get_comprehensive_status를 다시 호출하여 사용합니다.
