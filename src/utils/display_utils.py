...
def display_data_summary(
    updater: StockDataUpdater, include_backtest_analysis: bool = False
):
    """
    ...
    if include_backtest_analysis:
        ...
        logger.info(
            f"데이터 기간: {basic.get('date_range', ('N/A', 'N/A'))[0]} ~ {basic.get('date_range', ('N/A', 'N/A'))[1]}"
        )
        ...
        if backtest:
            ...
            logger.info(
                f"백테스팅 가능 종목: {backtest.get('valid_symbols_count', 0):,}개 ({backtest.get('valid_percentage', 0)}%)"
            )
            ...
            if top_symbols:
                ...
                for i, symbol_info in enumerate(top_symbols[:10], 1):
                    ...
                    logger.info(
                        f"  {i:2d}. {symbol}: {days}일 ({start_date} ~ {end_date})"
                    )
