...
def get_stock_data_for_backtest(symbol: str, start_date: str = None, end_date: str = None, days: int = 365) -> pd.DataFrame:
    """
    백테스팅용 주식 데이터 조회
    """
    conn = get_connection(DB_PATH)
    ...
    try:
        if start_date and end_date:
            ...
            start_date_calc = (datetime.now() - timedelta(days=days)).strftime('%Y-%m-%d')
            query = """
                SELECT date, open, high, low, close, volume
                FROM stock_data 
                WHERE symbol = ? AND date BETWEEN ? AND ?
...
def run_single_symbol_backtest(symbol: str, strategy_class, data: pd.DataFrame, initial_capital: float) -> Dict[str, Any]:
    """
    단일 종목 백테스팅 실행
    """
    try:
        ...
        total_value = capital + (position * data['close'].iloc[-1] if position > 0 else 0)
...
def run_backtest_ui(symbols: list, start_date: Optional[str] = None, end_date: Optional[str] = None, 
                   initial_capital: float = 1000000, strategy_name: str = "MACD 전략", **kwargs) -> Optional[Dict[str, Any]]:
    """
    실제 백테스트 UI 실행
    """
    try:
        ...
        for symbol in symbols:
            ...
            # 백테스팅 실행
            result = run_single_symbol_backtest(symbol, strategy_class, data, initial_capital)
        ...
        win_rates = [r['win_rate'] for r in all_results if 'error' not in r and r['win_rate'] > 0]
        sharpe_ratios = [r['sharpe_ratio'] for r in all_results if 'error' not in r and r['sharpe_ratio'] != 0]
        ...
        return {
            ...
            'trades': [
                {
                    '날짜': trade['date'].strftime('%Y-%m-%d') if hasattr(trade['date'], 'strftime') else str(trade['date']),
                    '종목': trade['symbol'],
                    '매매': trade['type'],
                    '가격': trade['price'],
                    '수량': trade['shares'],
