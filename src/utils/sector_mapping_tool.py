...
def _load_known_mappings(self) -> Dict[str, Dict[str, str]]:
        ...
        return {
            # KOSPI 주요 종목들
            "005930": {"sector_code": "013", "name": "삼성전자", "market": "KOSPI"},      # 전기전자
            "000660": {"sector_code": "013", "name": "SK하이닉스", "market": "KOSPI"},    # 전기전자  
            "035420": {"sector_code": "025", "name": "NAVER", "market": "KOSPI"},       # 서비스업
            "051910": {"sector_code": "008", "name": "LG화학", "market": "KOSPI"},       # 화학
            "028260": {"sector_code": "016", "name": "삼성물산", "market": "KOSPI"},      # 유통업
            "005380": {"sector_code": "015", "name": "현대차", "market": "KOSPI"},       # 운수장비
            "000270": {"sector_code": "015", "name": "기아", "market": "KOSPI"},         # 운수장비
            "068270": {"sector_code": "025", "name": "셀트리온", "market": "KOSPI"},     # 서비스업(바이오)
            "207940": {"sector_code": "013", "name": "삼성바이오로직스", "market": "KOSPI"}, # 전기전자
            "006400": {"sector_code": "013", "name": "삼성SDI", "market": "KOSPI"},      # 전기전자
            "012330": {"sector_code": "015", "name": "현대모비스", "market": "KOSPI"},   # 운수장비
            "030200": {"sector_code": "020", "name": "KT", "market": "KOSPI"},          # 통신업
            "017670": {"sector_code": "020", "name": "SK텔레콤", "market": "KOSPI"},     # 통신업
            "003550": {"sector_code": "020", "name": "LG유플러스", "market": "KOSPI"},   # 통신업
            "055550": {"sector_code": "022", "name": "신한지주", "market": "KOSPI"},     # 은행
            "105560": {"sector_code": "022", "name": "KB금융", "market": "KOSPI"},       # 은행
            "086790": {"sector_code": "022", "name": "하나금융지주", "market": "KOSPI"},  # 은행
            "032830": {"sector_code": "023", "name": "삼성생명", "market": "KOSPI"},      # 증권
            "018260": {"sector_code": "018", "name": "삼성에스디에스", "market": "KOSPI"}, # 건설업
            "090430": {"sector_code": "008", "name": "아모레퍼시픽", "market": "KOSPI"},  # 화학
            
            # KOSDAQ 주요 종목들
            "035720": {"sector_code": "107", "name": "카카오", "market": "KOSDAQ"},      # IT
            "066570": {"sector_code": "107", "name": "LG전자", "market": "KOSDAQ"},     # IT
            "096770": {"sector_code": "107", "name": "SK이노베이션", "market": "KOSDAQ"}, # IT
            "251270": {"sector_code": "108", "name": "넷마블", "market": "KOSDAQ"},      # 생명과학
            "247540": {"sector_code": "110", "name": "에코프로비엠", "market": "KOSDAQ"}, # 콘텐츠
            "058470": {"sector_code": "105", "name": "리노공업", "market": "KOSDAQ"},    # 소재
            "240810": {"sector_code": "106", "name": "원익IPS", "market": "KOSDAQ"},    # 부품장비
        }
...
def map_known_stocks(self) -> Dict[str, str]:
        ...
        for symbol, info in self.known_mappings.items():
            try:
                ...
                logger.info(f"종목 매핑 완료: {symbol} ({info['name']}) -> {info['sector_code']}")
...
def run_full_mapping(self, max_auto_map: int = 999999) -> Dict[str, any]:
        ...
        report = {
            ...
            "summary": {
                ...
                "total_success": len([r for r in known_results.values() if r.startswith("✅")]) + 
        ...
        logger.info("="*60)
        logger.info(f"✅ 대표 종목 매핑: {report['known_mappings']['success']}/{report['known_mappings']['total']}")
        logger.info(f"🤖 자동 매핑: {report['auto_mappings']['success']}/{report['auto_mappings']['total']}")
        logger.info(f"📋 총 처리: {report['summary']['total_success']}/{report['summary']['total_processed']}")
