    ...
    def get_symbol_update_status(self, symbol: str) -> Dict[str, Any]:
        ...
        with self.cache_lock:
            ...
            if cache_key in self.memory_cache:
                ...
                ).seconds < self.config.cache_expiry_hours * 3600:
...
class DataUpdateBatchProcessor:
    ...
    def create_optimized_batches(self, symbols: List[str]) -> List[List[str]]:
        ...
        current_memory_mb = self.memory_monitor["process"].memory_info().rss / (
            1024 * 1024
        )
        ...
        logger.info(
            f"배치 생성 완료: {len(batches)}개 배치, 배치 크기: {self.current_batch_size}"
        )
        return batches
    ...
    def _optimize_memory(self):
        ...
        if current_memory_mb > self.config.max_memory_usage_mb * 0.8:
            ...
            new_memory_mb = self.memory_monitor["process"].memory_info().rss / (
                1024 * 1024
            )
            logger.info(
                f"메모리 최적화: {current_memory_mb:.1f}MB -> {new_memory_mb:.1f}MB"
            )
...
class OptimizedDataUpdater:
    ...
    def update_symbols_optimized(
        ...
        performance_analysis = self._analyze_performance(
            parallel_results, total_time, len(symbols)
        )
    ...
    def _filter_symbols_by_cache(
        self, symbols: List[str], force_update: bool
    ) -> List[str]:
        """캐시 기반 종목 필터링"""
    ...
    def _update_batch_sequential(
        ...
        for symbol in batch:
            try:
                ...
                # 개별 종목 업데이트
                success = self._update_single_symbol_optimized(
    ...
    def _save_data_to_db(self, symbol: str, df: pd.DataFrame):
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                df_final.to_sql(
                    "stock_data", conn, if_exists="append", index=False, method="multi"
                )
    ...
    def _analyze_performance(
        self, parallel_results: Dict, total_time: float, total_symbols: int
    ) -> Dict[str, Any]:
        """성능 분석"""

        successful_updates = sum(
            1 for result in parallel_results["results"].values() if result
        )
        symbols_per_second = total_symbols / total_time if total_time > 0 else 0

        # 최적화 효율성 계산
        cache_efficiency = (
            cache_stats["hit_rate"] if cache_stats["total_requests"] > 0 else 0
        )
        skip_efficiency = (
            cache_stats["skipped_updates"] / total_symbols if total_symbols > 0 else 0
        )
        incremental_efficiency = cache_stats["incremental_updates"] / max(
            1, cache_stats["incremental_updates"] + cache_stats["full_updates"]
        )

        overall_efficiency = (
            cache_efficiency + skip_efficiency + incremental_efficiency
        ) / 3
        ...
        return {
            ...
            "success_rate": (
                successful_updates / total_symbols if total_symbols > 0 else 0
            ),
    ...
    def _update_performance_stats(
        self, symbols_count: int, total_time: float, performance_analysis: Dict
    ):
        """성능 통계 업데이트"""
        ...
        self.performance_stats["optimization_efficiency"] = performance_analysis[
            "overall_efficiency"
        ]
...
def create_optimized_data_updater(
    db_path: str,
    max_workers: int = 4,
    enable_cache: bool = True,
    batch_size: int = 30,
...
def progress_callback_with_eta(
    progress: float, completed: int, total: int, start_time: float = None
):
    """ETA 포함 진행률 콜백"""
    if start_time:
    ...
    print(
        f"\r📊 진행률: {progress:.1%} ({completed}/{total}){eta_str}",
        end="",
        flush=True,
    )
