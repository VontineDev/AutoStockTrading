...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(
            PROJECT_ROOT / "logs" / "test_results.log", encoding="utf-8"
        ),
    ],
)
...
def discover_and_run_tests(
    test_directory: str = "tests", verbosity: int = 2
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        logger.info(
            f"   성공: {test_results['tests_run'] - test_results['failures'] - test_results['errors']}"
        )
...
def run_specific_test_file(test_file: str, verbosity: int = 2) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"✅ {test_file} 테스트 완료: {test_results['tests_run']}개 실행, "
...
def run_test_coverage_analysis() -> Dict[str, Any]:
    ...
    try:
        ...
        return {"test_results": test_results, "coverage": coverage_report}

    except ImportError:
        logger.warning(
            "coverage 패키지가 설치되지 않음. pip install coverage로 설치하세요."
        )
...
def generate_test_report(results: Dict[str, Any]) -> str:
    ...
    if results.get("failures") or results.get("errors"):
        report_lines.extend(["🔍 실패/오류 상세:", ""])

        for failure in results.get("failure_details", []):
            report_lines.extend(["❌ 실패:", failure, ""])

        for error in results.get("error_details", []):
            report_lines.extend(["🚨 오류:", error, ""])
...
def main():
    ...
    parser.add_argument(
        "--file", "-f", help="특정 테스트 파일 실행 (예: test_constants.py)"
    )
    parser.add_argument(
        "--coverage", "-c", action="store_true", help="커버리지 분석 포함"
    )
    parser.add_argument(
        "--verbose",
        "-v",
    ...
    parser.add_argument(
        "--quick", "-q", action="store_true", help="빠른 실행 (상세도 1)"
    )
