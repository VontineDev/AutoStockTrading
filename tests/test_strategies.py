...
from src.strategies.base_strategy import (
    BaseStrategy, TradeSignal, StrategyConfig,
    create_default_config, calculate_signal_confidence
)
    ...
    def generate_signals(self, data: pd.DataFrame) -> List[TradeSignal]:
        ...
        for i in range(len(data)):
            ...
            if (row['sma_5'] > row['sma_20'] and 
                data.iloc[i-1]['sma_5'] <= data.iloc[i-1]['sma_20']):
                # 골든크로스 - 매수 신호
                signal = TradeSignal(
                    ...
                    confidence=0.8,
                    reason="Golden Cross",
                    indicators={'rsi': row['rsi'], 'sma_5': row['sma_5'], 'sma_20': row['sma_20']},
                    risk_level="MEDIUM"
                )
                signals.append(signal)
                
            elif (row['sma_5'] < row['sma_20'] and 
                  data.iloc[i-1]['sma_5'] >= data.iloc[i-1]['sma_20']):
                # 데드크로스 - 매도 신호
                signal = TradeSignal(
                    timestamp=datetime.now(),
...
class TestStrategyIntegration(unittest.TestCase):
    ...
    def test_multiple_signals_handling(self):
        ...
        trend_data = pd.DataFrame({
            'date': dates,
            'open': np.arange(50000, 50000 + 200 * 100, 100) + np.random.normal(0, 500, 200),
            'high': np.arange(50000, 50000 + 200 * 100, 100) + np.random.normal(1000, 500, 200),
            'low': np.arange(50000, 50000 + 200 * 100, 100) + np.random.normal(-1000, 500, 200),
            'close': np.arange(50000, 50000 + 200 * 100, 100) + np.random.normal(0, 300, 200),
        ...
        if len(signals) >= 2:
            ...
            self.assertTrue(any(s == "BUY" for s in signal_types) or 
                           any(s == "SELL" for s in signal_types))
