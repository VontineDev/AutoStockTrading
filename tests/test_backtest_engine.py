...
from src.trading.optimized_backtest import (
    OptimizedBacktestEngine, OptimizedBacktestConfig,
    create_optimized_engine, run_kospi_backtest_optimized
)
from src.strategies.base_strategy import BaseStrategy, TradeSignal, StrategyConfig
from src.constants import (
    TradingConstants, DataCollectionConstants, OptimizationConstants
)
...
class MockStrategy(BaseStrategy):
    ...
    def generate_signals(self, data: pd.DataFrame) -> list:
        ...
        for i in range(20, len(data)):  # 충분한 데이터가 있는 구간만
            ...
            # 골든크로스
            if (row['sma_5'] > row['sma_20'] and 
                prev_row['sma_5'] <= prev_row['sma_20']):
...
class TestOptimizedBacktestConfig(unittest.TestCase):
    ...
    def test_default_config(self):
        ...
        # 상수 기반 기본값 확인
        self.assertEqual(config.initial_capital, TradingConstants.DEFAULT_INITIAL_CAPITAL)
...
class TestOptimizedBacktestEngine(unittest.TestCase):
    ...
    def test_run_optimized_backtest_basic(self):
        ...
        results = self.engine.run_optimized_backtest(
            self.mock_strategy,
            self.test_symbols[:3],  # 작은 샘플로 테스트
            days=30
        )
...
class TestConvenienceFunctions(unittest.TestCase):
    ...
    def test_run_kospi_backtest_optimized(self, mock_get_kospi_top):
        ...
        # 데이터 수집기 모킹
        with patch('src.trading.optimized_backtest.StockCollector') as mock_collector_class:
            mock_collector_instance = MockDataCollector()
            mock_collector_class.return_value = mock_collector_instance
            
            try:
                results = run_kospi_backtest_optimized(
                    MockStrategy,
                    top_n=3,
                    days=30
                )
                ...
                self.assertIn('StockCollector', str(e)) or self.assertIn('get_kospi_top', str(e))
...
class TestPerformanceOptimizations(unittest.TestCase):
    ...
    def test_parallel_processing_benefit(self):
        ...
        # 결과 검증
        self.assertEqual(len(sequential_results['results']), len(parallel_results['results']))
    ...
    def test_batch_processing_memory_efficiency(self):
        ...
        try:
            results = engine.run_optimized_backtest(
                MockStrategy,
                large_symbol_list[:10],  # 적당한 크기로 테스트
                days=15
            )
    ...
    def test_optimization_components_integration(self):
        ...
        # 첫 번째 실행 (캐시 구축)
        results1 = engine.run_optimized_backtest(
            MockStrategy, symbols, days=25
        )
        
        # 두 번째 실행 (캐시 활용)
        results2 = engine.run_optimized_backtest(
            MockStrategy, symbols, days=25
        )
