...
from scripts.data_update import StockDataUpdater # StockDataUpdater를 직접 임포트하여 모의 객체로 대체 가능성 열어둠
...
def setup_test_db():
    ...
    with patch('scripts.data_update.StockDataUpdater.db_path', new_callable=lambda: db_path):
        # StockDataUpdater의 __init__에서 _init_database가 호출되므로,
        # 테스트용 DB 경로로 초기화되도록 Mocking
        with patch('scripts.data_update.StockDataUpdater._init_database') as mock_init_db:
...
def test_load_stock_data(MockStockDataManager, MockStockDataUpdater, setup_test_db):
    ...
    assert all(col in data["005930"].columns for col in ['date', 'open', 'high', 'low', 'close', 'volume'])
    ...
    # 데이터가 없는 종목 테스트
    mock_dm_instance.get_latest_data.return_value = pd.DataFrame() # 빈 DataFrame 반환하도록 설정
...
def test_calculate_ta_indicators():
    ...
    data = {
        'date': pd.to_datetime(pd.date_range(start='2023-01-01', periods=100)), # 30 -> 100
    ...
    for col in expected_indicator_cols:
        ...
        assert df_with_indicators[col].isnull().sum() < len(df_with_indicators) * 0.5 # 조건 완화

    # 빈 DataFrame 테스트
    empty_df = pd.DataFrame(columns=['date', 'open', 'high', 'low', 'close', 'volume'])
    empty_df_with_indicators = calculate_ta_indicators(empty_df)
    assert empty_df_with_indicators.empty
    assert 'rsi' not in empty_df_with_indicators.columns # 빈 DF에는 지표 컬럼이 추가되지 않아야 함
...
def test_create_candlestick_chart():
    ...
    chart_with_indicator = create_candlestick_chart(df, symbol, indicators=['test_indicator'])
