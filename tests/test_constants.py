...
def test_worker_counts_valid(self):
    ...
    self.assertGreaterEqual(
        DataCollectionConstants.DEFAULT_MAX_WORKERS, DataCollectionConstants.MIN_WORKERS
    )
    self.assertLessEqual(
        DataCollectionConstants.DEFAULT_MAX_WORKERS, DataCollectionConstants.MAX_WORKERS
    )
...
def test_trading_signals_valid(self):
    ...
    # 신호 값들이 서로 다른지 확인
    signals = {
        TradingConstants.BUY_SIGNAL,
        TradingConstants.SELL_SIGNAL,
        TradingConstants.HOLD_SIGNAL,
    }
...
def test_optimization_thresholds(self):
    ...
    self.assertLessEqual(
        OptimizationConstants.MIN_SYMBOLS_FOR_PARALLEL,
        OptimizationConstants.MIN_SYMBOLS_FOR_BATCH,
    )
    self.assertLessEqual(
        OptimizationConstants.MIN_SYMBOLS_FOR_CACHING,
        OptimizationConstants.MIN_SYMBOLS_FOR_BATCH,
    )
...
def test_timeout_settings(self):
    ...
    self.assertLess(
        OptimizationConstants.TASK_TIMEOUT, OptimizationConstants.THREAD_POOL_TIMEOUT
    )
...
def test_day_trading_characteristics(self):
    ...
    self.assertLessEqual(
        day_trading["rsi_period"], StrategyPresets.SWING_TRADING["rsi_period"]
    )
...
def test_constants_consistency(self):
    ...
    # 트레이딩 상수 일관성
    self.assertLess(
        TradingConstants.STOP_LOSS_PERCENT, TradingConstants.MAX_DRAWDOWN_PERCENT
    )

    # 데이터 수집 일관성
    self.assertLess(
        DataCollectionConstants.DEFAULT_BACKTEST_DAYS,
...
def test_trading_constants_dict(self):
    ...
    self.assertEqual(
        TRADING_CONSTANTS["DEFAULT_INITIAL_CAPITAL"],
        TradingConstants.DEFAULT_INITIAL_CAPITAL,
    )
    self.assertEqual(
        TRADING_CONSTANTS["TRANSACTION_FEE_RATE"], TradingConstants.TRANSACTION_FEE_RATE
    )
...
def test_talib_constants_dict(self):
    ...
    self.assertEqual(TALIB_CONSTANTS["RSI_PERIOD"], TALibConstants.RSI_PERIOD)
    self.assertEqual(
        TALIB_CONSTANTS["MACD_FAST_PERIOD"], TALibConstants.MACD_FAST_PERIOD
    )
...
def test_system_constants_dict(self):
    ...
    self.assertEqual(
        SYSTEM_CONSTANTS["DEFAULT_DB_NAME"], SystemConstants.DEFAULT_DB_NAME
    )
    self.assertEqual(
        SYSTEM_CONSTANTS["DEFAULT_MAX_WORKERS"],
