...
│   └── main.py            # 메인 진입점 (통합 관리)
├── scripts/
│   ├── data_update.py     # 데이터 수집 스크립트
│   ├── check_data_status_legacy.py  # 레거시 데이터 확인 도구 (마이그레이션용)
│   └── utils/             # 유틸리티 함수들
├── streamlit_app/         # 웹 앱 메인
├── data/                  # SQLite 데이터베이스
...
- **`scripts/`**: 독립 실행 스크립트 및 유틸리티
  - `check_data_status_legacy.py`: 마이그레이션 도구 (곧 제거 예정)
- **`streamlit_app/`**: 웹 인터페이스
- **`data/`**: SQLite 데이터베이스 및 캐시
- **`backtest_results/`**: 백테스팅 결과 파일들
